...
useEffect(() => {
  ...
  <a className="cursor-pointer flex items-center">
      <span className={`self-center text-2xl font-semibold whitespace-nowrap dark:text-white ${activePage === 'home' ? 'text-emerald-700 dark:text-emerald-400' : ''}`} onClick={() => {
        navigate('/')
        setActivePage('home')
      }}>FC SHOP</span>
  </a>
  ...
  <div className="flex md:order-2">
    <div className="relative">
      ...
      {isActive === true ? (
        <div className='bg-white absolute bottom-auto rounded-lg dark:bg-gray-700 dark:text-white w-52 h-64 border border-emerald-700 flex flex-col overflow-y-scroll'>
          {results.map((e) => (
          ...
          })} className='p-2 hover:bg-emerald-700 dark:text-white hover:text-white text-black border-b cursor-pointer border-gray-400'>{e.title}</div>
        ))}
    ...
    <div className='flex items-center ml-5 cursor-pointer ' onClick={() => {
      ...
      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`w-6 h-6 dark:text-white ${activePage === 'cart' ? 'dark:text-emerald-500 text-emerald-700' : ''}`}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
      </svg>
    </div>

    <button id="theme-toggle" onClick={darkModeHandler} type="button" className="sm:ml-2 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 rounded-lg text-sm p-2.5">
  ...
  <div className="bg-emerald-500 dark:bg-emerald-800 text-center text-white cursor-pointer">
    <ul className="flex flex-row justify-evenly">
    <li className={`hover:bg-emerald-700 w-full ${activePage === `men's clothing` ? 'bg-emerald-600' : ''}`} onClick={() => {
        navigate('/category')
        setActivePage(`men's clothing`)
      }}>Men</li>
      <li className={`hover:bg-emerald-700 w-full ${activePage === `women's clothing` ? 'bg-emerald-600' : ''}`} onClick={() => {
        navigate('/category')
        setActivePage(`women's clothing`)
      }}>Women</li>
      <li className={`hover:bg-emerald-700 w-full ${activePage === 'electronics' ? 'bg-emerald-600' : ''}`} onClick={() => {
        navigate('/category')
        setActivePage('electronics')
      }}>Electronics</li>
      <li className={`hover:bg-emerald-700 w-full ${activePage === 'jewelery' ? 'bg-emerald-600' : ''}`} onClick={() => {
        navigate('/category')
