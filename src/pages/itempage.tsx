            ...
            -200 shadow-md rounded-lg m-10 p-5 flex sm:flex-row flex flex-col dark:border-red-300 items-center">
                ...
                <div className="flex flex-col justify-between h-full pl-5">
                    <div className="flex flex-row">
                        ...
                        <span className="bg-blue-100 text-blue-800 text-s font-semibold mr-2 h-full px-2.5 py-2.5 rounded dark:bg-red-200 dark:text-red-800 ml-3">{itemRating.rate}</span>
                    </div>
                    <div className="w-3/4"><h1>{itemDescription}</h1></div>
                    <div className="flex flex-row justify-between">
                        <h1 className="text-3xl">${itemPrice}</h1>
                        <button onClick={() => onClickHandler(state)} className="dark:bg-red-700 dark:hover:bg-red-800 mt-2 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-lg px-5 py-2.5 text-center dark:focus:ring-red-400">Add to cart</button>
                    </div>
                </div>
            </div>
        </div>
